...
- as sugestões de novas funcionalidades apresentadas em texto devem ser pertinentes e acrescentar funcionalidades relevantes ao *software*
- o levantamento de *bugs* deve
- as alterações de código submetidas pelos utilizadores não devem provocar alterações significantes no comportamento do código nem regressões nos testes de compatibilidade; será apenas aceite código devidamente formatado e organizado segundo um determinado estilo e formatação que devem ser obedecidos (também definidos pelo proprietário no documento CONTRIBUTING.md)
