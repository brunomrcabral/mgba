...
O organizador deste projeto enumera uma série de ... : que serão apenas aceites se o código estiver devidamente formatado e organizado segundo uma determinada nomenclatura e estilo de código que devem ser obedecidos (também definidas pelo proprietário no documento). 

Numa fase inicial, os *pull requests* e as alterações submetidas pelos utilizadores estão sujeitas a um processo automático de validação antes de serem aprovados. Após uma análise da estrutura do repositório, verificou-se que este foi configurado para utilizar a ferramenta Travis-CI na verificação de conflitos que possam surgir entre o código da versão presente no *master* e o código submetido nos *pull requests* e na realização de testes de compilação automáticos sobre o *source code* submetido, rejeitando código em conflito ou que apresente erros de compilação.

Existe ainda um processo de análise e validação manual feita pelo proprietário deste projeto:
- as sugestões apresentadas em texto devem ser pertinentes e acrescentar funcionalidades relevantes ao *software*
- as alterações de código submetidas pelos utilizadores não devem provocar alterações significantes no comportamento do código nem regressões nos testes de compatibilidade.
...
No período de um mês compreendido entre 18 de setembro e 18 de outubro de 2015 foram recebidos apenas 2 pull requests, o que ilustra a clara necessidade de uma seleção criteriosa destes pedidos.
