...
Apesar das vantagens que foram referidas nos parágrafos anteriores, após uma análise cuidadosa do repositório do **mGBA** realizada pelos elementos do grupo, concluimos que o proprietário deste projeto não tira partido máximo das funcionalidades disponibilizadas pelo **Travis-CI** aos seus utilizadores, pois não recorre a esta ferramenta para executar testes unitários sobre as alterações no código. Pelo contrário, o autor do **mGBA** serve-se apenas dela para realizar **testes de compilação**. Para acrescentar ao que foi referido anteriormente, na raiz do repositório encontrámos ainda dois ficheiros relacionados com esta ferramenta, designados ".travis.yml" e ".travis-deps.sh".
