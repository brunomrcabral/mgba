...
###Isolabilidade
A maior parte das classes que fazem parte do *package* principal *gba* faz uso de classes e métodos pertencentes a outros *packages*, estando algumas destas intimamente ligadas e interdependentes entre si, como é o caso da relação **arm <=> gba**, cujas componentes não podem ser isoladas em nenhuma circunstância. Numa situação destas seria correto afirmar que ao testarmos uma componente de um determinado recurso do programa estamos também a testar indiretamente outras componentes presentes em diferentes módulos.
...
Em projetos de dimensões médias como este, a existência de documentação e comentários no código, bem como a sua boa estruturação apresentam uma importância acrescida no entendimento do funcionamento de cada componente do *software* a ser desenvolvido. No caso de um emulador, a documentação permitiria também aos novos *developers* aprofundar os conhecimentos que possuem da plataforma a ser emulada pela máquina virtual. É da responsabilidade do **proprietário** e dos seus colaboradores promover a consistência e qualidade do código, bem como manter a sua documentação atualizada e acessível a qualquer outro indivíduo que demonstre interesse em participar no projeto.

Consideramos que o projeto em estudo perde bastante nestes aspetos que acabaram de ser referidos, pois revela-se **pouco detalhado** na descrição das classes e das funções nelas existentes, revelando até uma completa ausência de documentação em grande parte dos ficheiros de código fonte, sendo pouco clara quando esta existia, o que torna certos módulos do programa de **difícil compreensão**, dificuldade sentida no geral por todos os elementos do grupo.

###Heterogeneidade
Com a constante evolução e maturação do **mGBA**, bem como o aparecimento de novas tecnologias e paradigmas de programação cresce também a necessidade de diversificar os métodos de teste aplicados ao *software* de modo a conseguirem abranger todas as novas funcionalidades recentemente implementadas. A heterogeneidade deste projeto deve-se não só ao número de bibliotecas utilizadas e funcionalidades implementadas, mas também às potencialidades de expansão deste projeto para novas plataformas.

Como podemos constatar pela presença de determinados ficheiros na raiz do repositório, o projeto **mGBA** utiliza várias tecnologias e utilitários de terceiros (*third-party*) que permitem aos colaboradores e utilizadores finais compilar o projeto de forma rápida e eficiente, independentemente da plataforma e dos compiladores em uso, sendo estas:
...
- **Travis-CI :** ferramenta para realização de testes de integração contínua *open source* distribuído de forma gratuita aos utilizadores do **GitHub**.
